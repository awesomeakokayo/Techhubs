...
export async function POST(req: Request, { params }: { params: { trackId: string } }) {
  ...
  let enrollment = await prisma.guidedPathEnrollment.findUnique({ where: { userId_trackId: { userId: session.user.id, trackId: params.trackId } } })
  if (!enrollment) enrollment = await prisma.guidedPathEnrollment.create({ data: { userId: session.user.id, trackId: params.trackId } })
  if (stepIndex !== enrollment.currentStepIndex) return NextResponse.json({ error: 'Complete the currently unlocked step before continuing.' }, { status: 409 })
  const completedStep = steps[stepIndex]
  if (!completedStep) return NextResponse.json({ error: 'Step not found' }, { status: 404 })
  enrollment = await prisma.guidedPathEnrollment.update({ where: { userId_trackId: { userId: session.user.id, trackId: params.trackId } }, data: { currentStepIndex: stepIndex + 1 } })
  const existing = await prisma.userProgress.findFirst({ where: { userId: session.user.id, trackId: params.trackId, itemType: completedStep.type === 'project' ? 'project' : 'stage', stageId: completedStep.stageId ?? undefined, resourceId: completedStep.resourceId || completedStep.projectId || undefined } })
  if (!existing) await prisma.userProgress.create({ data: { userId: session.user.id, trackId: params.trackId, itemType: completedStep.type === 'project' ? 'project' : 'stage', stageId: completedStep.stageId, resourceId: completedStep.resourceId || completedStep.projectId, status: 'COMPLETED', completedAt: new Date() } })
  if (stepIndex === steps.length - 1) await prisma.courseCompletion.upsert({ where: { userId_trackId: { userId: session.user.id, trackId: params.trackId } }, create: { userId: session.user.id, trackId: params.trackId, completedAt: new Date() }, update: { completedAt: new Date() } })
