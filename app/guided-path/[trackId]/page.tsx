...
export default function GuidedPathPage() {
  ...
  const quizResult = useMemo(() => {
    if (currentStep?.type !== 'quiz' || !submitted || !currentStep.quizQuestions?.length) return { correctCount: 0, total: 0, score: 0, requiredCorrect: 0, passed: false }
    const total = currentStep.quizQuestions.length
    const correctCount = currentStep.quizQuestions.reduce((count, question, index) => count + (answers[`${currentStep.index}-${index}`] === question.correctIndex ? 1 : 0), 0)
    const isAI = trackId.startsWith('ai-')
    const requiredCorrect = isAI ? Math.ceil(total * 0.8) : total
    return { correctCount, total, score: Math.round((correctCount / total) * 100), requiredCorrect, passed: correctCount >= requiredCorrect }
  }, [answers, currentStep, submitted, trackId])
...
function StepCard({ step, answerMap, submitted, quizResult, onSelect, onSubmitQuiz, onRetryQuiz, onComplete }: { step: GuidedStep; answerMap: Record<string, number>; submitted: boolean; quizResult: { correctCount: number; total: number; score: number; requiredCorrect: number; passed: boolean }; onSelect: (key: string, value: number) => void; onSubmitQuiz: () => void; onRetryQuiz: () => void; onComplete: () => void }) {
  ...
  const isAIQuiz = step.type === 'quiz' && step.stageId != null
