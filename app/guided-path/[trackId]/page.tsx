...
export default function GuidedPathPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/guided-path/${trackId}`)
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Failed to load your guided path.')
          setLoading(false)
        }
      })
  ...
  async function completeStep(stepIndex: number) {
    ...
    if (step?.type === 'quiz') {
      payload.answers = step.quizQuestions?.map((_, index) => answers[`${step.index}-${index}`]) ?? []
    }

    const response = await fetch(`/api/guided-path/${trackId}`, {
      method: 'POST',
  ...
  if (currentIndex >= steps.length) {
    return (
      <div className="max-w-2xl mx-auto py-20 px-6 text-center">
        <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full" style={{ background: 'rgba(22,163,74,0.1)' }}>
          <Trophy size={30} style={{ color: 'var(--color-success)' }} />
        </div>
        ...
        <div className="mt-8 flex justify-center gap-3">
          <Link href={`/certificate/${trackId}`} className="btn btn-primary inline-flex items-center gap-2">Get certificate <ArrowRight size={16} /></Link>
          <Link href={track ? `/tracks/${track.slug}` : '/tracks'} className="btn btn-secondary">Back to track</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-10">
      ...
      <div className="mt-6 flex items-center gap-2">
        <span className="badge inline-flex items-center gap-1"><GraduationCap size={12} /> PRO</span>
        <span className="text-xs text-text-muted">Guided Path · Cross-device sync</span>
      </div>

      <div className="mt-6">
        <p className="section-label">YOUR LEARNING PATH</p>
        <div className="progress-bar-container mt-3"><div className="progress-bar-fill" style={{ width: `${progress}%` }} /></div>
        <p className="mt-2 text-sm text-text-secondary">Step {currentIndex + 1} of {steps.length} · {progress}% complete</p>
      </div>
      ...
      <StepCard
        ...
        onSelect={(key, value) => setAnswers((previous) => ({ ...previous, [key]: value }))}
        onSubmitQuiz={() => { setError(''); setSubmitted(true) }}
        onRetryQuiz={() => { setAnswers({}); setSubmitted(false); setError('') }}
        onComplete={() => completeStep(currentIndex)}
      />

      <div className="mt-6 space-y-3">
        {steps.slice(currentIndex + 1, currentIndex + 4).map((step) => <div key={step.index} className="card opacity-40 flex items-center gap-3"><Lock size={16} className="text-text-muted" /><div><p className="text-sm font-medium text-text-secondary">{step.title}</p><p className="text-xs text-text-muted">Complete current step to unlock</p></div></div>)}
      </div>
...
function StepCard({ step, answerMap, submitted, allQuizCorrect, onSelect, onSubmitQuiz, onRetryQuiz, onComplete }: {
  step: GuidedStep
  ...
  const typeMeta = step.type === 'concept'
    ? { icon: BookOpen, label: 'STAGE', color: 'var(--accent-tertiary)' }
    : step.type === 'resource'
      ? { icon: FileText, label: 'RESOURCE', color: 'var(--accent-primary)' }
      : step.type === 'quiz'
        ? { icon: Sparkles, label: 'VERIFY', color: 'var(--accent-secondary)' }
        : step.type === 'project'
          ? { icon: Code2, label: 'PROJECT', color: 'var(--color-success)' }
          : { icon: Trophy, label: 'MILESTONE', color: 'var(--color-info)' }
  ...
  return (
    <section className="card mt-8">
      <div className="flex flex-wrap items-center gap-2">
        <span className="badge inline-flex items-center gap-1" style={{ background: `${typeMeta.color}15`, color: typeMeta.color }}><Icon size={12} /> {typeMeta.label} · {step.estimatedTime}</span>
        {resourceMeta && <span className="badge inline-flex items-center gap-1"><resourceMeta.icon size={12} /> {resourceMeta.label}</span>}
        {step.resourceFree && <span className="badge" style={{ color: 'var(--color-success)' }}>Free</span>}
        {step.resourceSource && <span className="text-xs text-text-muted">{step.resourceSource}</span>}
      </div>
