...
export const QUIZ_DATA: Record<string, QuizQuestion[]> = {
  ...
  // ─── AI CATEGORY ───
  'ai-f1': [
    { question: 'What is the difference between machine learning and generative AI?', options: ['They are the same thing', 'Machine learning is a broad field; generative AI is a specific application that creates new content', 'Generative AI is older than machine learning', 'Machine learning only works with text'], correctIndex: 1, explanation: 'Machine learning is a broad field of AI where systems learn from data. Generative AI is a specific application that creates new content (text, images, code) based on patterns learned from training data.' },
    { question: 'Which of the following is NOT something AI is generally good at?', options: ['Summarizing long documents', 'Performing calculations on large datasets', 'Making high-stakes medical decisions without human review', 'Brainstorming ideas based on patterns'], correctIndex: 2, explanation: 'AI should not be the sole authority for high-stakes decisions like medical, legal, or financial choices. These require qualified human judgment alongside AI assistance.' },
  ],
  'ai-f3': [
    { question: 'What is a large language model (LLM)?', options: ['A database that stores large amounts of text', 'A neural network trained on text data that can generate and understand language', 'A search engine for the internet', 'A programming language'], correctIndex: 1, explanation: 'An LLM is a type of neural network trained on massive amounts of text data, enabling it to generate, understand, and reason about human language.' },
    { question: 'Why might AI generate incorrect information confidently?', options: ['The AI is deliberately lying', 'AI generates text based on patterns, not verified facts, so it can produce plausible-sounding but incorrect output', 'The internet connection is slow', 'The AI model is too small'], correctIndex: 1, explanation: 'AI models generate text based on statistical patterns in training data. They do not verify facts in real-time, so they can produce output that sounds confident but is factually wrong — this is called hallucination.' },
  ],
  'ai-f5': [
    { question: 'What makes a good AI prompt?', options: ['Being as short as possible', 'Including context, audience, format, constraints, and examples', 'Using the word "please" and "thank you"', 'Writing in all caps for emphasis'], correctIndex: 1, explanation: 'Good prompts establish context (what you need), audience (who it is for), format (how you want the output), constraints (length, style), and examples (if helpful).' },
    { question: "A manager asks an AI tool for current market statistics. The answer contains exact numbers but provides no sources. What is the best next step?", options: ['Ask the AI the same question again', 'Add "be 100% accurate" to the prompt', 'Verify the figures using current authoritative sources', 'Assume the numbers are correct because they are specific'], correctIndex: 2, explanation: 'Specific or confident AI output is not evidence of accuracy. Current factual claims should be verified using authoritative sources.' },
  ],
  'ai-f6': [
    { question: 'Which instruction gives an AI system the most useful context?', options: ['"Write something good."', '"Explain this."', '"Write a 300-word explanation for a first-year computer science student who has never used an API. Include one simple example."', '"Give me the smartest answer possible."'], correctIndex: 2, explanation: 'The instruction establishes audience (CS student), purpose (explain API), length (300 words), subject context (never used an API), and expected output (one example).' },
    { question: 'You need to analyze a dataset containing 100,000 transaction records. Which approach is most appropriate?', options: ['Paste all 100,000 records into a chatbot', 'Ask the chatbot to estimate the totals', 'Use an appropriate data-analysis/database workflow and use AI where it helps with analysis or explanation', 'Ask the AI to identify the answer from a random sample and assume the same result'], correctIndex: 2, explanation: 'Large structured datasets are better handled by appropriate computational/data tools. AI can assist with analysis, transformation, explanation, and workflow design.' },
  ],
  'ai-s1': [
    { question: 'An AI coding agent modifies several files and reports that all tests pass. You inspect the diff and notice that it changed authentication logic unrelated to your original task. What should you do?', options: ['Accept the changes because the tests passed', 'Accept the changes because an AI agent knows the codebase', 'Review the changes, revert unrelated modifications, and verify the intended behavior independently', 'Delete the project and start again'], correctIndex: 2, explanation: 'Passing tests do not automatically make unrelated changes acceptable. AI-generated changes still require human review and scope control.' },
    { question: 'What is the most important practice when using AI coding tools?', options: ['Accept all generated code without reviewing it', 'Review and understand every piece of code the AI generates', 'Use the most expensive AI model available', 'Never test AI-generated code'], correctIndex: 1, explanation: 'AI coding tools can generate code quickly, but you must review and understand every piece of code to ensure correctness, security, and maintainability.' },
  ],
}
